...
# Required radiative channels.
#
# IMPORTANT:
# radiative_rates.csv contains idx_upper/idx_lower collisions for bundled rows.
# Therefore, DO NOT identify A-values by idx_upper/idx_lower alone.
# Use the physical resolved transition selector (type=res_to_res, n,l -> n,l),
# while using the state_index_upper for multiplying the CR population vector.
#
# Tuple format:
#   (state_index_upper, state_index_lower, n_upper, l_upper, n_lower, l_lower, label)
LINE_CHANNELS = {
    "Halpha": [
        (IDX_3S, IDX_2P, 3, 0, 2, 1, "3S_to_2P"),
        (IDX_3P, IDX_2S, 3, 1, 2, 0, "3P_to_2S"),
        (IDX_3D, IDX_2P, 3, 2, 2, 1, "3D_to_2P"),
    ],
    "Hbeta": [
        (IDX_4S, IDX_2P, 4, 0, 2, 1, "4S_to_2P"),
        (IDX_4P, IDX_2S, 4, 1, 2, 0, "4P_to_2S"),
        (IDX_4D, IDX_2P, 4, 2, 2, 1, "4D_to_2P"),
    ],
}
...
def load_radiative_weights(use_photon_energy: bool = False) -> LineWeights:
    ...
    required_cols = {
        "type", "n_upper", "l_upper", "n_lower", "l_lower",
        "label_upper", "label_lower", "A_s-1", "idx_upper", "idx_lower"
    }
    ...
    for line, channels in LINE_CHANNELS.items():
        ...
        for state_upper, state_lower, n_u, l_u, n_l, l_l, label in channels:
            rows = df[
                (df["type"] == "res_to_res")
                & (df["n_upper"] == n_u)
                & (df["l_upper"] == l_u)
            ...
            if rows.empty:
                raise ValueError(
                    f"No resolved radiative row found for {label}: "
                    f"type=res_to_res, ({n_u},{l_u})->({n_l},{l_l})"
                )
            if len(rows) != 1:
                raise ValueError(
                    f"Expected exactly one resolved row for {label}; found {len(rows)}.\n"
                    f"Rows:\n{rows.to_string(index=False)}"
                )

            row = rows.iloc[0]
            A_val = float(row["A_s-1"])

            # Sanity check: the resolved row should map to the CR state indices.
            csv_upper = int(row["idx_upper"])
            csv_lower = int(row["idx_lower"])
            if csv_upper != state_upper or csv_lower != state_lower:
                raise ValueError(
                    f"State-index mismatch for {label}: CSV idx=({csv_upper},{csv_lower}), "
                    f"expected=({state_upper},{state_lower}). This means state ordering changed."
                )
...
def print_weights(line_weights: LineWeights, use_photon_energy: bool) -> None:
    ...
    for line, channels in LINE_CHANNELS.items():
        ...
        for _state_upper, _state_lower, _n_u, _l_u, _n_l, _l_l, label in channels:
...
def line_intensity(n: np.ndarray, line: str, line_weights: LineWeights) -> float:
    total = 0.0
    for state_upper, _state_lower, _n_u, _l_u, _n_l, _l_l, label in LINE_CHANNELS[line]:
        total += line_weights.weights[line][label] * n[state_upper]
    return float(total)
...
def plot_single_ratio_transient(res: TransientResult) -> None:
    ...
    ax2.set_ylabel("signed Hα/Hβ error vs post-step QSS [%]")
...
def plot_single_absolute_errors(res: TransientResult) -> None:
    ...
    ax.set_ylabel("signed fractional error vs post-step QSS [%]")
    ax.set_title(
...
def plot_overlay_ratio_errors(results: List[TransientResult]) -> None:
    ...
    ax.set_xlabel("time after step [s]")
    ax.set_ylabel("signed Hα/Hβ error vs post-step QSS [%]")
    if results:
